...
public class TrackerClient {
	...
	public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName)
		...
		try {
			...
			ProtoCommon.RecvPackageInfo pkgInfo =
				ProtoCommon.recvPackage(trackerSocket.getInputStream(), ProtoCommon.TRACKER_PROTO_CMD_RESP,
			...
			port =
				(int)ProtoCommon.buff2long(pkgInfo.body,
	...
	public StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName)
		...
		try {
			...
			ProtoCommon.RecvPackageInfo pkgInfo =
				ProtoCommon.recvPackage(trackerSocket.getInputStream(), ProtoCommon.TRACKER_PROTO_CMD_RESP, -1);
			this.errno = pkgInfo.errno;
			if (pkgInfo.errno != 0) {
				return null;
			}
	...
	public StorageServer getFetchStorage(TrackerServer trackerServer, String groupName, String filename)
		throws IOException {
		ServerInfo[] servers =
			this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE, groupName, filename);
		if (servers == null) {
			return null;
		} else {
	...
	public StorageServer getUpdateStorage(TrackerServer trackerServer, String groupName, String filename)
		throws IOException {
		ServerInfo[] servers =
			this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_UPDATE, groupName, filename);
		if (servers == null) {
			return null;
		} else {
			return new StorageServer(servers[0].getIpAddr(), servers[0].getPort(), 0);
...
//		byte store_path;
	...
	public StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName, String storageIpAddr)
		...
		try {
			...
			header =
				ProtoCommon.packHeader(ProtoCommon.TRACKER_PROTO_CMD_SERVER_LIST_STORAGE,
	...
	private boolean deleteStorage(TrackerServer trackerServer, String groupName, String storageIpAddr)
		...
		header =
			ProtoCommon.packHeader(ProtoCommon.TRACKER_PROTO_CMD_SERVER_DELETE_STORAGE,
				ProtoCommon.FDFS_GROUP_NAME_MAX_LEN + ipAddrLen, (byte)0);
		byte[] wholePkg = new byte[header.length + bGroupName.length + ipAddrLen];
		System.arraycopy(header, 0, wholePkg, 0, header.length);
		System.arraycopy(bGroupName, 0, wholePkg, header.length, bGroupName.length);
		...
		out.write(wholePkg);
		ProtoCommon.RecvPackageInfo pkgInfo =
			ProtoCommon.recvPackage(trackerSocket.getInputStream(), ProtoCommon.TRACKER_PROTO_CMD_RESP, 0);
		this.errno = pkgInfo.errno;
